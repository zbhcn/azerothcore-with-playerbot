            ...
            if (!anyPlayersInCombat && mapABInfo->combatLocked)
            {
                ...
                if (mapABInfo->combatLockTripped)
                {
                    for (auto player : mapABInfo->allMapPlayers)
                    {
                        ...
                        {
                            ChatHandler(player->GetSession()).PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000 战斗结束. 不再锁定难度.|r");
                        }
                    }
                }
                    ...
                    if (mapABInfo->playerCount)
                    {
                        for (std::vector<Player*>::const_iterator playerIterator = mapABInfo->allMapPlayers.begin(); playerIterator != mapABInfo->allMapPlayers.end(); ++playerIterator)
                        {
                            ...
                            if (thisPlayer)
                            {
                                ...
                                {
                                    chatHandle.PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000 欢迎来到%s(%u个玩家%s). 有%u个玩家在副本里. 难度设置为%u玩家.|r",
                                        map->GetMapName(),
                                        instanceMap->GetMaxPlayers(),
                                        instanceDifficulty,
                                        mapABInfo->playerCount,
                                    ...
                                    {
                                        chatHandle.PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000你的GM标记已经打开. 副本难度将忽略你. 请关闭GM后重进.|r");
                                    }
                                }
                                else
                                {
                                    ...
                                    if (!player->IsGameMaster())
                                    {
                                        chatHandle.PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000 %s 进入副本. 有%u个玩家在副本里. 难度设置为%u玩家.|r",
                    ...
                    if (mapABInfo->playerCount)
                    {
                        for (std::vector<Player*>::const_iterator playerIterator = mapABInfo->allMapPlayers.begin(); playerIterator != mapABInfo->allMapPlayers.end(); ++playerIterator)
                        {
                            ...
                            if (thisPlayer && thisPlayer != player)
                            {
                                ...
                                if (mapABInfo->combatLocked)
                                {
                                    chatHandle.PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000 %s 在战斗进行时离开副本. 难度锁定不少于%u玩家直到战斗结束.|r",
                                        player->GetName().c_str(),
                                        mapABInfo->adjustedPlayerCount
                                    );
                                }
                                ...
                                {
                                    chatHandle.PSendSysMessage("|cffc3dbff [动态副本]|r|cffFF8000 %s 离开了副本. 有%u个玩家在副本里，难度设定为%u个玩家.|r",
                                        player->GetName().c_str(),
                                        mapABInfo->playerCount,
                                        mapABInfo->adjustedPlayerCount
                                    );
