...
PlayerbotSecurityLevel PlayerbotSecurity::LevelFor(Player* from, DenyReason* reason, bool ignoreGroup)
{
    ...
    //解除对立阵营限制
    //if (botAI->IsOpposing(from))//禁止自动同意组队 && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP))
    //{
    //    if (reason)
    ...
    //}
    //解除对立阵营限制
    if (sPlayerbotAIConfig->IsInRandomAccountList(account))//禁止自动同意组队 && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP))
    {
            ...
            case PLAYERBOT_DENY_GEARSCORE:
                {
                    ...
                    out << "你的装等太低了: |cffff0000" << fromGS << "|cffffffff/|cff00ff00" << botGS << " |cffff0000" << diff << "%|cffffffff/|cff00ff00" << req << "%";
                }
                break;
