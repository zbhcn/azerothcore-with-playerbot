...
bool LootAction::Execute(Event event)
{
    ...
    //std::unique_lock<std::mutex> lock(LootMutex); // 加锁
...
uint32 stackCount = urand(1, proto->GetMaxStackSize());
    ...
    LOG_ERROR("playerbots", "AhBot {} added {} of {} to auction {} for {}..{}", bot->GetName().c_str(), stackCount, proto->Name1.c_str(), 1, bidPrice, buyoutPrice);
...
bool StoreLootAction::IsLootAllowed(uint32 itemid, PlayerbotAI* botAI)
{
    ...
    if (!context) return false; // 检查 context 是否为空

    LootStrategy* lootStrategy = AI_VALUE(LootStrategy*, "loot strategy");
    if (!botAI) return false; // 检查 botAI 是否为空

    ItemTemplate const* proto = sObjectMgr->GetItemTemplate(itemid);
    if (!proto) return false; // 检查 proto 是否为空
