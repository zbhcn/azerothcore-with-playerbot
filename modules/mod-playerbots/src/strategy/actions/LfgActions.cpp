...
bool LfgJoinAction::JoinLFG()
{
    ...
    bool heroic = urand(0, 100) < 50 && (visitor.count[ITEM_QUALITY_EPIC] >= 0 || visitor.count[ITEM_QUALITY_RARE] >= 0) && bot->getLevel() >= 70;
...
bool LfgTeleportAction::Execute(Event event)
{
    ...
    bot->ClearUnitState(UNIT_STATE_ALL_STATE);
    //botAI->ResetStrategies(0);
    sLFGMgr->TeleportPlayer(bot, out);

    //随机本自动传送到主人身边
    if (bot->GetMapId() && botAI->GetMaster() && bot->GetMap() && bot->GetMapId() && botAI->GetMaster()->IsInWorld() &&
        botAI->GetMaster()->GetMapId() && bot->GetGroup() && botAI->GetMaster()->IsAlive() &&
        !bot->GetMap()->IsDungeon() && !bot->IsBeingTeleported() && !botAI->GetMaster()->IsBeingTeleported() &&
        bot->GetGroup()->isLFGGroup() && bot->GetMapId() != botAI->GetMaster()->GetMapId()
        && botAI->GetMaster()->GetMap() && botAI->GetMaster()->GetMap()->IsDungeon())
    {
        if (bot->isDead())
        {
        ...
        bot->TeleportTo(botAI->GetMaster()->GetMapId(), botAI->GetMaster()->GetPositionX(), botAI->GetMaster()->GetPositionY(), botAI->GetMaster()->GetPositionZ(), 0);
        botAI->TellMaster("我进随机本了");
