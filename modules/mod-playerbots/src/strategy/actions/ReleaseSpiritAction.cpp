...
bool ReleaseSpiritAction::Execute(Event event)
{
    if (bot->IsAlive())
    {
        botAI->TellMasterNoFacing("我没有死，会在这里等着");
    ...
    {
        botAI->TellMasterNoFacing("我已经是一个精灵了");
        return false;
    }

    WorldPacket& p = event.getPacket();
    if (!p.empty() && p.GetOpcode() == CMSG_REPOP_REQUEST)
        botAI->TellMasterNoFacing("正在释放灵魂...");

    if(botAI->GetMaster() && !botAI->GetMaster()->IsAlive())
        botAI->TellMasterNoFacing("在墓地等我");
