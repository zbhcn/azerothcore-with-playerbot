...
bool FollowAction::Execute(Event event)
{
    ...
    {
        if(std::abs(bot->GetPositionZ() - botAI->GetMaster()->GetPositionZ()) > 3 && botAI->GetMaster()->GetMapId() &&
        bot->GetMapId() == botAI->GetMaster()->GetMapId() && botAI->GetMaster()->GetMap() && botAI->GetMaster()->GetMap()->IsDungeon())
...
bool FleeToMasterAction::Execute(Event event)
{
    ...
    {
        if (!urand(0, 3))
            botAI->TellMaster("怕ADD我来了！");
    }
    else if (distance < 1000)
    {
        if (!urand(0, 10))
            botAI->TellMaster("我正前往你的位置");
    }
    else
        if (!urand(0,20))
            botAI->TellMaster("我正在靠近你.");
