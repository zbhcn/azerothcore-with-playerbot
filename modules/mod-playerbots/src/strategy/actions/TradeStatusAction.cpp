...
{
    if (!bot->GetTrader() || GET_PLAYERBOT_AI(bot->GetTrader()))
        return;
    ...
    if (sRandomPlayerbotMgr->IsRandomBot(bot))
    {
        ...
        if (discount)
        {
            ...
            out << "折扣最高可达: " << chat->formatMoney(discount);
...
bool TradeStatusAction::CheckTrade()
{
    ...
    if (!botAI->HasActivePlayerMaster() && GET_PLAYERBOT_AI(bot->GetTrader()))
    {
        ...
        if (isGettingItem)
        {
            if (bot->GetGroup() && bot->GetGroup()->IsMember(bot->GetTrader()->GetGUID()) && botAI->HasRealPlayerMaster())
                ...
                bot->Say("谢谢 " + chat->FormatWorldobject(bot->GetTrader()), (bot->GetTeamId() == TEAM_ALLIANCE ? LANG_COMMON : LANG_ORCISH));
        }
    ...
    for (uint32 slot = 0; slot < TRADE_SLOT_TRADED_COUNT; ++slot)
    {
        ...
        {
            std::ostringstream out;
            out << chat->FormatItem(item->GetTemplate()) << " - 这不是卖的";
            botAI->TellMaster(out);
    ...
    std::ostringstream out;
    out << "我只能出 " << chat->formatMoney(-(delta + discount)) << " 购买";
    botAI->TellMaster(out);
    botAI->PlaySound(TEXT_EMOTE_NO);
    return false;
}
