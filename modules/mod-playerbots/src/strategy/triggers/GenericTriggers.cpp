...
{
    if (!bot->IsInWorld() || bot->IsBeingTeleported() || (bot->GetMap() && bot->GetMap()->IsDungeon() || bot->GetMap()->IsRaid()))
        return false;

    if (bot->GetMap() && (bot->GetMap()->IsDungeon() || bot->GetMap()->IsRaid() || !bot->IsInWorld()))
        return false;
