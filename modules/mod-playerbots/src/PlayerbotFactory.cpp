...
void PlayerbotFactory::InitEquipment(bool incremental)
{
    ...
    for (uint8 slot = 0; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        //禁止删除钥匙
        if (oldItem && oldItem->GetTypeId()!= 13)
        {
...
void PlayerbotFactory::InitArenaTeam()
{
    ...
    if (arenateam->GetMembersSize() < ((uint32)arenateam->GetType() * 2) && bot->getLevel() >= 70)
    {
        ...
        if ((bot && bot->GetArenaTeamId(arenateam->GetSlot())) || (bot && sCharacterCache->GetCharacterArenaTeamIdByGuid(bot->GetGUID(), arenateam->GetSlot()) != 0))
            return;
