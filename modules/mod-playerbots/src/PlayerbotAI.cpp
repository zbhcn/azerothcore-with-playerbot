...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    if (bot && bot->GetGroup() && !bot->InBattleground())
    {
        ...
        if (group && leader && (leader->isAFK() || !leader->GetSession() || GET_PLAYERBOT_AI(leader) || !HasRealPlayerMaster() || !master || !master->IsInWorld()))
        {
            bot->RemoveFromGroup();
            GET_PLAYERBOT_AI(bot)->SetMaster(nullptr);
            ResetStrategies();
            if(!bot->IsBeingTeleported())
            bot->TeleportTo(530, -1919.67, 5544.15, -12.42, 2.237);//传送回沙塔斯
            if (bot->GetMap() && !bot->GetMap()->IsRaidOrHeroicDungeon())//重置副本CD
            {
                for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
                {
                    const BoundInstancesMap& binds = sInstanceSaveMgr->PlayerGetBoundInstances(bot->GetGUID(), Difficulty(i));
                    if (binds.empty())
                        continue;
        ...
        if(bot->GetMap() && bot->GetMap()->IsDungeon() && !bot->GetGroup() && !bot->InBattleground() && !bot->IsBeingTeleported())
            bot->TeleportTo(530, -1919.67, 5544.15, -12.42, 2.237);//传送回沙塔斯
        //自动传送到主人身边
        if (bot->GetGroup() && bot->GetMap())
        {
            ...
            if (leader && leader->IsInWorld() && leader->IsAlive()
                && leader->GetZoneId() != bot->GetZoneId() && !bot->IsBeingTeleported() && !master->IsBeingTeleported())
            {
                if (bot->isDead())
                {
                ...
                bot->TeleportTo(leader->GetMapId(), leader->GetPositionX(), leader->GetPositionY(), leader->GetPositionZ(), 0);
...
void PlayerbotAI::DoNextAction(bool min)
{
    ...
    //            break; // 找到一个助理就设定为 master，然后退出循环
    //        }
    //    }
    //}
    // test BG master set
	...
	for (uint32 auraType = SPELL_AURA_BIND_SIGHT; auraType < TOTAL_AURAS; auraType++)
	{
		...
		for (AuraEffect const* aurEff : auras)
		{
			...
			if (IsRealAura(bot, aurEff, unit))
			{
                ...
                if (maxStack) {
                    if (aurEff && maxStackAmount && aurEff->GetBase()->GetStackAmount() >= maxStackAmount)
                        auraAmount++;

                    if (aurEff && maxProcCharges && aurEff->GetBase()->GetCharges() >= maxProcCharges)
                        auraAmount++;
                } else {
                    auraAmount++;
                }
