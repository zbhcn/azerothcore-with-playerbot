        ...
        do
        {
            ...
            for (uint8 cls = CLASS_WARRIOR; cls < MAX_CLASSES; ++cls)
            {
                for (uint32 spec = 1; spec < 5; ++spec)
                {
                    ...
                    {
                        // 在每次迭代中创建一个新的 stat 对象
                        WeightScaleStat newStat;
                        newStat.stat = stat.stat; // 复制原始 stat 的值
                        newStat.weight = stat.weight; // 复制原始 stat 的值
                        m_weightScales[cls][spec].stats.push_back(std::move(newStat));
                    }
                }
            }
            statcount++;
