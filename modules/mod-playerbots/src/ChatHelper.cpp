...
std::string const ChatHelper::FormatItem(ItemTemplate const* proto, uint32 count, uint32 total)
{
    ...
    //snprintf(color, sizeof(color), "%08x", ItemQualityColors[proto->Quality]);
    const std::string& name = sObjectMgr->GetItemLocale(proto->ItemId)->Name[sWorld->GetDefaultDbcLocale()];

    //const ItemLocale* itemLocale = sObjectMgr->GetItemLocale(proto->ItemId);
    //if (!itemLocale) {
    //    return "未知物品"; // 或者抛出异常
    //}
    //// 提取装备名称为中文名称,可能宕机,容器中第5个元素为中文
    //const std::vector<std::string> localname = itemLocale->Name;
    //const std::string chineseName = localname.size() > 4 ? localname[4] : "";
...
std::string const ChatHelper::FormatQItem(uint32 itemId)
{
    ...
    snprintf(color, sizeof(color), "%08x", ItemQualityColors[0]);
...
GuidVector ChatHelper::parseGameobjects(std::string const text)
{
    ...
    while (true)
    {
        ...
        try
        {
            guid = std::stoull(text.substr(pos, endPos - pos));
        }
        catch (const std::invalid_argument&)
        {
            break; // if conversion fails, break
        }
        catch (const std::out_of_range&)
        {
            break; // if conversion is out of range, break
        }
        ...
        try
        {
            entry = std::stoul(text.substr(pos, endPos - pos));
        }
        catch (const std::invalid_argument&)
        {
            break; // if conversion fails, break
