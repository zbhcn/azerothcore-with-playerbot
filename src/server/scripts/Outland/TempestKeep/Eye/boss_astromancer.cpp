        ...
        scheduler.Schedule(3650ms, [this](TaskContext context)
            {
            ...
            }).Schedule(21800ms, [this](TaskContext context)
                {
                    ...
                    }).Schedule(52100ms, [this](TaskContext context)//安排了一个初始任务，在52.1秒后执行
                        {
                            me->SetReactState(REACT_PASSIVE);
                            scheduler.DelayAll(20s);//原来为22秒
                            ...
                            scheduler.Schedule(1s, [this](TaskContext)//安排了一个1秒后的任务来召唤光点
                                {
                                    for (uint8 i = 0; i < 3; ++i)
                                    {
                                ...
                                }).Schedule(2s, [this](TaskContext)//召唤光点任务之后，再安排一个2秒后的任务进行对话
                                    {
                                        Talk(SAY_SUMMON);
                                    }).Schedule(3s, [this](TaskContext)//安排一个3秒后的任务来清除BOSS所有光环并隐藏模型
                                        {
                                            me->RemoveAllAuras();
                                            me->SetModelVisible(false);
                                        }).Schedule(7s, [this](TaskContext)//安排一个7秒后的任务来召唤小怪和显形模型
                                            {
                                                bool summoned = false;
                                                ...
                                                if (summoned)
                                                {
                                                    me->SetModelVisible(true); // 确保在召唤小怪后模型显形
                                                }
                                            }).Schedule(9s, [this](TaskContext)//安排一个23秒(改成9秒)后的任务恢复攻击状态并继续调度
                                                {
                                                    me->SetReactState(REACT_AGGRESSIVE);
                                                    summons.DoForAllSummons([&](WorldObject* summon)
                                                        {
                                            ...
                                            context.Repeat(87500ms, 91200ms);//重复整个过程，每87.5到91.2秒循环一次
                        });
    }
