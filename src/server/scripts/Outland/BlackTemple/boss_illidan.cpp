...
//{
//    {639.97f, 301.63f, 370.08f, 0.0f},
//    {658.83f, 265.10f, 370.08f, 0.0f},
//    {656.86f, 344.07f, 370.08f, 0.0f},
//    {640.70f, 310.47f, 370.08f, 0.0f},
//
//    {706.22f, 273.26f, 370.08f, 0.0f},
//    {717.55f, 328.33f, 370.08f, 0.0f},
//    {718.06f, 286.08f, 370.08f, 0.0f},
//    {705.92f, 337.14f, 370.08f, 0.0f}
//};
const Position eyeBeamPos[MAX_EYE_BEAM_POS * 2] =
{
    {710.8815f, 306.4028f, 353.5962f, 0.0f},
    {652.105f,  259.5127f, 353.0794f, 0.0f},
    {642.7164f, 305.2436f, 353.5596f, 0.0f},
    {710.8815f, 306.4028f, 353.5962f, 0.0f},

    {660.3492f, 345.5749f, 353.2961f, 0.0f},
    {701.6755f, 297.3358f, 354.041f, 0.0f},
    {706.7507f, 269.4593f, 353.2778f, 0.0f},
    {660.3492f, 345.5749f, 353.2961f, 0.0f}
};

const Position airHoverPos[MAX_EYE_BEAM_POS] =
{
    {658.83f, 265.10f, 380.08f, 3.961900f},
    {706.22f, 273.26f, 380.08f, 2.251072f},
    {705.92f, 337.14f, 380.08f, 0.850345f},
    {656.86f, 344.07f, 380.08f, 5.235990f}
};
...
//            && unit->GetDistance2d(unit->GetPositionX(), BladesPositions[0].GetPositionY()) > 35.0f
//            && unit->GetDistance2d(BladesPositions[1].GetPositionX(), BladesPositions[1].GetPositionY()) > 35.0f;
//    }
//};
...
public:
    ...
    struct boss_illidan_stormrageAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            me->RemoveUnitFlag(UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);//修复不可选中
        }
        ...
        void ScheduleNormalEvents(uint8 phase)
        {
            ...
            events.ScheduleEvent(EVENT_SHEAR, 12000);
        }
        ...
        void JustEngagedWith(Unit* who) override
        {
            ...
            //float flameBlastTimer = 10000; // 重置计时器
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            case EVENT_PHASE_2_EYE_BEAM_START:
                //me->SetDisableGravity(true);//确保悬浮
                //me->SendMovementFlagUpdate();
                Talk(SAY_ILLIDAN_EYE_BLAST);
                if (Creature* trigger = summons.GetCreatureWithEntry(NPC_ILLIDAN_DB_TARGET))
                    trigger->GetMotionMaster()->MovePoint(0, eyeBeamPos[beamPosId + MAX_EYE_BEAM_POS], false, true);//eyeBeamPos[beamPosId + MAX_EYE_BEAM_POS],
                break;
                //case EVENT_SPELL_FLAME_BURST:
                //    if (summons.HasEntry(NPC_FLAME_OF_AZZINOTH))
                //    {
                //        if (Creature* flame = summons.GetCreatureWithEntry(NPC_FLAME_OF_AZZINOTH))
                //        {
                //            if (flame->GetVictim() && !flame->HasUnitState(UNIT_STATE_CASTING))
                //            {
                //                flame->CastSpell(flame->GetVictim(), SPELL_FLAME_BLAST);
                //                flame->CastSpell(flame->GetVictim(), SPELL_BLAZE_SUMMON);
                //            }
                //        }
                //    }
                //    events2.ScheduleEvent(EVENT_SPELL_FLAME_BURST, 10000);
                //    break;
            ...
            case EVENT_START_PHASE_2_END:
                ...
                if (!summons.HasEntry(NPC_FLAME_OF_AZZINOTH))
                {
                    ...
                    events2.Reset();
                    me->InterruptNonMeleeSpells(false);
                    me->GetMotionMaster()->MovePoint(POINT_ILLIDAN_MIDDLE, 676.02f, 305.45f, 380.08f);//353.6f
                }
                else
                {
            ...
            case EVENT_OUTRO_3:
                ...
                return; // 确保对象销毁后立即返回，避免访问已经销毁的对象
            }
            ...
            case EVENT_PHASE_2_SUMMON1:
                ...
                me->SetDisableGravity(true);//确保悬浮
                //me->SendMovementFlagUpdate();
                me->GetMotionMaster()->MovePoint(POINT_ILLIDAN_HOVER, 727.875f, 305.365f, 380.08f, false, true);
                break;
            ...
            case EVENT_PHASE_4_START:
                ...
                //events.ScheduleEvent(EVENT_REMOVE_DEMON_FORM, 60000);//减半时间
                events.ScheduleEvent(EVENT_SPELL_FLAME_BURST, 10500);
                events.ScheduleEvent(EVENT_SPELL_SHADOW_DEMONS, 18000);
                events.ScheduleEvent(EVENT_REMOVE_DEMON_FORM, 30000);
                break;
            ...
            case EVENT_SPELL_FLAME_BURST:
                ...
                events.ScheduleEvent(EVENT_SPELL_FLAME_BURST, 11000);
                break;
...
public:
    ...
    struct npc_akama_illidanAI : public npc_escortAI
    {
        ...
        void DoAction(int32 param) override
        {
            ...
            {
                events.Reset();
                me->RemoveAllAuras();//防止卡战斗
                summons.DespawnAll();
                me->CombatStop(true);
                me->SendClearThreatListOpcode();//防止卡战斗
            }
...
public:
    ...
    class spell_illidan_shadow_prison_SpellScript : public SpellScript
    {
        ...
        {
            // Ensure targets list is not empty and contains valid objects
            if (targets.empty())
                return;

            // Remove invalid targets (e.g., nullptr)
            targets.remove_if([](WorldObject* target) { return !target; });

            // Apply the PlayerOrPetCheck safely
...
//
//    struct npc_shadow_demonAI : public CreatureAI
//    {
//        npc_shadow_demonAI(Creature* creature) : CreatureAI(creature), _instance(creature->GetInstanceScript()) {}
//        void Reset() override
//        {
...
//    CreatureAI* GetAI(Creature* creature) const override
//    {
//        return GetBlackTempleAI<npc_shadow_demonAI>(creature);
//    }
//};
