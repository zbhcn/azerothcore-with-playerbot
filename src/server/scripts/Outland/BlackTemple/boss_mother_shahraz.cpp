        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case EVENT_SPELL_FATAL_ATTRACTION:
                    ...
                    events.ScheduleEvent(EVENT_SPELL_FATAL_ATTRACTION, 30000);//改30秒
                    break;
            }
...
public:
    ...
    class spell_mother_shahraz_fatal_attraction_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            // 随机化目标列表
            std::vector<WorldObject*> targetVec(targets.begin(), targets.end());
            std::random_device rd;
            std::mt19937 g(rd());
            std::shuffle(targetVec.begin(), targetVec.end(), g);

            // 将随机化后的目标列表复制回原始列表
            targets.assign(targetVec.begin(), targetVec.end());
            // 确保传送不超过3人
            if (targets.size() > 3)
                targets.resize(3);
        }
...
public:
    ...
    class spell_mother_shahraz_fatal_attraction_dummy_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_mother_shahraz_fatal_attraction_dummy_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);//SPELL_EFFECT_DUMMY
        }
    };
...
public:
    ...
    class spell_mother_shahraz_fatal_attraction_aura_AuraScript : public AuraScript
    {
        ...
        void Register() override
        {
            //OnEffectApply += AuraEffectApplyFn(spell_mother_shahraz_fatal_attraction_aura_AuraScript::OnApply, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
