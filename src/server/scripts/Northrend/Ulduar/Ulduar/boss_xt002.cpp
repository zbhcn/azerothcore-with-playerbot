...
public:
    ...
    struct npc_xt002_heartAI : public PassiveAI
    {
        ...
        {
            if (spellInfo->Id == SPELL_ENERGY_ORB_TRIGGER && target->GetEntry() == NPC_PILE_TRIGGER && me->GetDistance(target) > 60.0f)
            {
                std::vector<Creature*> validPiles;

                // 遍历所有召唤的生物，找到满足条件的 NPC_PILE_TRIGGER
                for (SummonList::const_iterator itr = summons.begin(); itr != summons.end(); ++itr)
                {
                    if (Creature* summon = ObjectAccessor::GetCreature(*me, *itr))
                    {
                        if (summon->GetEntry() == NPC_PILE_TRIGGER && me->GetDistance(summon) > 60.0f)
                        {
                            validPiles.push_back(summon);
                        }
                    }
                }

                // 如果找到的有效 pile 位置超过两个，按距离降序排序并选择最远的两个
                if (validPiles.size() > 2)
                {
                    std::sort(validPiles.begin(), validPiles.end(), [this](Creature* a, Creature* b) {
                        return me->GetDistance(a) > me->GetDistance(b);
                        });

                    validPiles.resize(2);  // 只保留最远的两个
                }

                switch (_spawnSelection)
                {
                case 0:
                    for (auto* pile : validPiles)
                    {
                        me->SummonCreature(NPC_XS013_SCRAPBOT, pile->GetPositionX() + irand(-3, 3), pile->GetPositionY() + irand(-3, 3), pile->GetPositionZ() + 2, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000);
                    }
                    _spawnSelection++;
                    break;
                case 1:
                    if (!validPiles.empty())
                    {
                        me->SummonCreature(NPC_XE321_BOOMBOT, validPiles[0]->GetPositionX(), validPiles[0]->GetPositionY(), validPiles[0]->GetPositionZ() + 2, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                    }
                    _spawnSelection++;
                    break;
                case 2:
                    ...
                    break;
                case 3:
                    if (!validPiles.empty() && _pummelerCount < 2)
                    {
                        me->SummonCreature(NPC_XM024_PUMMELLER, validPiles[0]->GetPositionX(), validPiles[0]->GetPositionY(), validPiles[0]->GetPositionZ() + 2, 0, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000);
                        _pummelerCount++;
                    }
                    _spawnSelection++;
                    break;
