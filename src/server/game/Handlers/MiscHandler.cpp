...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    data << uint32(matchCount * 3);     // 在线人数增加3倍，placeholder, count of players matching criteria
    //data << uint32(onlineCountToSend);   // 在线人数，如果有条件则发送真实在线人数，否则发送在线人数乘以3
    ...
    // 判断玩家是否发送了条件
    bool hasConditions = (levelMin != 0 || strCount != 0);
    // 根据条件判断在线人数
    uint32 onlineCountToSend = hasConditions ? matchCount : matchCount * 3;

    data.put(0, displaycount);                            // insert right count, count displayed
    //data.put(4, matchCount * 3);                         // 在线人数增加3倍insert right count, count of matches
    data.put(4, onlineCountToSend);                     // insert right count, count displayed
